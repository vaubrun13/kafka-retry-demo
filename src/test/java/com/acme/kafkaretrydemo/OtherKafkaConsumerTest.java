...
import com.acme.kafkaretrydemo.consumers.OtherKafkaConsumer;
...
public class OtherKafkaConsumerTest {
  ...
  private static final String TOPIC = "my-demo-topic-async-retries";
  ...
  @Autowired private OtherKafkaConsumer consumer;
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              assertAll(
                  () -> assertThat(consumer.deadMessages).containsKey(id),
                  () -> verify(businessService, times(3)).updateSomething(expectedRequestObject));
            });
